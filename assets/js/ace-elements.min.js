...
! function(a, b) {
    var c = function(c, d) {
        ...
        function f(b) { b.preventDefault(), b.stopPropagation(), cb = bb = v ? b.pageY : b.pageX, R = !0, a("html").off("mousemove.ace_scroll").on("mousemove.ace_scroll", g), a(S).off("mouseup.ace_scroll").on("mouseup.ace_scroll", h), B.addClass("active"), T && m.$element.trigger("drag.start") }

        function g(a) { a.preventDefault(), a.stopPropagation(), cb = v ? a.pageY : a.pageX, cb - bb + H > I ? cb = bb + I - H : 0 > cb - bb + H && (cb = bb - H), H += cb - bb, bb = cb, 0 > H ? H = 0 : H > I && (H = I), m.update_scroll() }

        function h(b) { b.preventDefault(), b.stopPropagation(), R = !1, a("html").off(".ace_scroll"), a(S).off(".ace_scroll"), B.removeClass("active"), T && m.$element.trigger("drag.end"), x && X && !Z && j() }
        ...
        function j() { null != db && (clearTimeout(db), db = null), db = setTimeout(function() { db = null, B.removeClass("not-idle") }, Y) }
        ...
        this.create = function(b) {
            if (!y) { b && (o = a.extend({}, a.fn.ace_scroll.defaults, b)), this.size = parseInt(this.$element.attr("data-size")) || o.size || 200, v = !o.horizontal, p = v ? "top" : "left", q = v ? "height" : "width", r = v ? "maxHeight" : "maxWidth", s = v ? "clientHeight" : "clientWidth", t = v ? "scrollTop" : "scrollLeft", u = v ? "scrollHeight" : "scrollWidth", this.$element.addClass("ace-scroll"), "static" == this.$element.css("position") ? (Q = this.element.style.position, this.element.style.position = "relative") : Q = !1;
                ...
                X && B.addClass("idle-hide"), $ && B.on("mouseenter.ace_scroll", function() { Z = !0, i(!1) }).on("mouseleave.ace_scroll", function() { Z = !1, 0 == R && j() }), this.$element.on("mouseenter.ace_scroll touchstart.ace_scroll", function() { ab = !0, $ ? i(!0) : o.hoverReset && m.reset(!0), B.addClass("scroll-hover") }).on("mouseleave.ace_scroll touchend.ace_scroll", function() { B.removeClass("scroll-hover") }), v || z.children(0).css(q, this.size), z.css(r, this.size), w = !1, y = !0 } }, this.is_active = function() {
            return x }, this.is_enabled = function() {
            return !w }, this.move_bar = function(a) { K = a }, this.get_track = function() {
            return D }, this.reset = function(a) {
            if (!w) { y || this.create();
                var b = this.size;
                if (!a || ab) {
                    if (ab = !1, V) {
    ...
    var c = function(b, c) {
        ...
        k.on(ace.click_event, function(b) {
            ...
            if ("number" == typeof c) {
                ...
                b.selectedIndex = c, k.find("a:eq(" + c + ")").trigger(ace.click_event) } else if ("string" == typeof c) {
                ...
                k.find("a:eq(" + c + ")").trigger(ace.click_event) } }, this.destroy = function() { f.removeClass("hide").off("change.color").next().remove(), j = [] } };
...
function(a, b) {
    ...
    var h = function() {
        ...
        j = function(b) {
            var c = this,
                ...
                e = new a.Deferred,
                h = function(b) { d.prepend("<img class='middle' style='display:none;' />");
                    var c = d.find("img:last").get(0);
                    a(c).one("load", function() { i.call(null, c) }).one("error", function() { j.call(null, c) }), c.src = b },
                i = function(b) {
                    var f = 50; "large" == c.settings.thumbnail ? f = 150 : "fit" == c.settings.thumbnail && (f = d.width()), d.addClass(f > 50 ? "large" : "");
                    var h = k(b, f);
                    if (null == h) return a(this).remove(), void e.reject({ code: g.error.THUMBNAIL_FAILED });
                    var i = h.w,
                        j = h.h; "small" == c.settings.thumbnail && (i = j = f), a(b).css({ "background-image": "url(" + h.src + ")", width: i, height: j }).data("thumb", h.src).attr({ src: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==" }).show(), e.resolve() },
                j = function() { d.find("img").remove(), e.reject({ code: g.error.IMAGE_LOAD_FAILED }) };
            if (f && b instanceof File) {
                var l = new FileReader;
                l.onload = function(a) { h(a.target.result) }, l.onerror = function() { e.reject({ code: g.error.FILE_LOAD_FAILED }) }, l.readAsDataURL(b) } else b instanceof Object && b.hasOwnProperty("path") && h(b.path);
            return e.promise() },
        k = function(b, c) {
            var d = b.width,
                e = b.height;
            ...
            try {
                ...
                h.drawImage(b, 0, 0, b.width, b.height, 0, 0, d, e), f = g.toDataURL() } catch (i) { f = null }
    ...
    b.prototype = { constructor: b, select: function() {
            ...
            return this.$menu.hide(), this.shown = !1, this }, lookup: function() {
            var b;
            return this.query = this.$element.val(), !this.query || this.query.length < this.options.minLength ? this.shown ? this.hide() : this : (b = a.isFunction(this.source) ? this.source(this.query, a.proxy(this.process, this)) : this.source, b ? this.process(b) : this) }, process: function(b) {
            var c = this;
            ...
            return b = a(b).map(function(b, d) {
                return b = a(c.options.item).attr("data-value", d), b.find("a").html(c.highlighter(d)), b[0] }), b.first().addClass("active"), this.$menu.html(b), this }, next: function() {
    ...
    a.fn.bs_typeahead = function(c) {
        ...
        return a.fn.bs_typeahead = c, this }, a(document).on("focus.bs_typeahead.data-api", '[data-provide="bs_typeahead"]', function() {
        var b = a(this);
        b.data("bs_typeahead") || b.bs_typeahead(b.data()) }) }(window.jQuery),
function(a) { a.fn.ace_wysiwyg = function(b) {
        var c = a.extend({ speech_button: !0, wysiwyg: {} }, b),
        ...
        return this.each(function() {
            ...
            for (var d in f)
                if (f.hasOwnProperty(d)) {
                    var g = f[d];
                    if (null === g) { b += ' </div> <div class="btn-group"> ';
                        continue }
                    ...
                    switch (g.name) {
                        ...
                        case "foreColor":
                        case "backColor":
                            b += ' <select class="hide wysiwyg_colorpicker" title="' + g.title + '"> ', a.each(g.values, function(a, c) { b += ' <option value="' + c + '">' + c + "</option> " }), b += " </select> ", b += ' <input style="display:none;" disabled class="hide" type="text" data-edit="' + g.name + '" /> ';
                            break;
                        case "viewSource":
            ...
            c.speech_button && "onwebkitspeechchange" in (k = document.createElement("input")) && (b += ' <input class="wysiwyg-speech-input" type="text" data-edit="inserttext" x-webkit-speech />'), k = null, b += " </div> ", b = c.toolbar_place ? c.toolbar_place.call(this, b) : a(this).before(b).prev(), b.find("a[title]").tooltip({ animation: !1, container: "body" }), b.find(".dropdown-menu input[type=text]").on("click", function() {
                return !1 }).on("change", function() { a(this).closest(".dropdown-menu").siblings(".dropdown-toggle").dropdown("toggle") }).on("keydown", function(b) { 27 == b.which ? (this.value = "", a(this).change()) : 13 == b.which && (b.preventDefault(), b.stopPropagation(), a(this).change()) }), b.find("input[type=file]").prev().on(ace.click_event, function() { a(this).next().click() }), b.find(".wysiwyg_colorpicker").each(function() { a(this).ace_colorpicker({ pull_right: !0 }).change(function() { a(this).nextAll("input").eq(0).val(this.value).change() }).next().find(".btn-colorpicker").tooltip({ title: this.title, animation: !1, container: "body" }) });
            var l = a(this),
                m = !1;
            ...
            a(this).wysiwyg(n) }), this } }(window.jQuery),
function(a, b) {
    function c(b, c) {
    ...
    a.fn.ace_spinner = function(d, e) {
        ...
        return f === b ? g : f }, a.fn.ace_spinner.defaults = { icon_up: "fa fa-chevron-up", icon_down: "fa fa-chevron-down", on_sides: !1, btn_up_class: "", btn_down_class: "", max: 999, touch_spinner: !1 }
}(window.jQuery),
function(a) { a.fn.aceTree = a.fn.ace_tree = function(b) {
...
function(a) { a.fn.aceWizard = a.fn.ace_wizard = function(b) {
        return this.each(function() {
            var c = a(this);
...
function(a, b) {
    ...
    var d = a(window);
    a(document).on("show.bs.modal", ".modal.aside", function() { a(".aside.in").modal("hide"), a(this).ace_aside("show") }).on("hide.bs.modal", ".modal.aside", function() { a(this).ace_aside("hide") }).on("shown.bs.modal", ".modal.aside", function() { a(this).ace_aside("shown") }).on("hidden.bs.modal", ".modal.aside", function() { a(this).ace_aside("hidden") }), a(window).on("resize.aside_container", function() { a(".modal.aside").ace_aside("insideContainer") }), a(document).on("settings.ace.aside", function(b, c) { "main_container_fixed" == c && a(".modal.aside").ace_aside("insideContainer") }), a.fn.aceAside = a.fn.ace_aside = function(d, e) {
        var f, g = this.each(function() {
            var b = a(this),
                g = b.data("ace_aside"),
                h = "object" == typeof d && d;
