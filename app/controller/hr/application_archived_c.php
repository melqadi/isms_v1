...
if ($res[0] == 1) {
    ?> 
        <meta http-equiv="refresh" content="0;url=<?php echo $dir?>?sec=hr&mod=application_archived&action=application_archived"/>
              <?php 
...
$res = $db->getResult();
    ...
    $db = new Database();
$db->connect();
$db->update( $tab,array('date'=>"",'action'=>""),$conditions); // Table name, column names and values, WHERE conditions
$res = $db->getResult();
}
    }  
...
$db = new Database();
$db->connect();
$db->selectall("application",'*',null,"`action` = 'desactive'"); // Table name, Column Names, JOIN, WHERE conditions, ORDER BY conditions
$res = $db->getResult();
       } 
        else
            {
...
$db = new Database();
$db->connect();
$db->selectall($tab,'*',null,'id_'.$tab.'='.$id_application); // Table name, Column Names, JOIN, WHERE conditions, ORDER BY conditions
$res = $db->getResult();
