...
if (isset($_GET['accept']) && $_GET['accept'] == 1)
    {
    $conditions='id_application='.$_GET['id_application'];

    $db = new Database();
$db->connect();
$db->update( $tab,array('status'=>'accept'),$conditions); // Table name, column names and values, WHERE conditions
$res = $db->getResult();
...
echo $not; 
?> 
                <meta http-equiv="refresh" content="0;url=<?php echo $dir?>?sec=hr&mod=application&action=application"/>
...
/*1*/$a_school = $_POST['a_school'];
/*1*/$a_postion = $_POST['a_position'];
/*1*/$a_start = $_POST['a_start'];
/*1*/$p_salary = $_POST['p_salary'];
/*1*/$e_salary = $_POST['e_salary'];

$db = new Database();
$db->connect();

$id_categoriess = $db->escapeString($id_categories); // Escape any input before insert
$id_poss = $db->escapeString($id_pos); // Escape any input before insert
$id_departmentss = $db->escapeString($id_departments); // Escape any input before insert
$a_schools = $db->escapeString($a_school); // Escape any input before insert
$a_postions = $db->escapeString($a_postion); // Escape any input before insert
$a_starts = $db->escapeString($a_start); // Escape any input before insert
$p_salarys = $db->escapeString($p_salary); // Escape any input before insert
$e_salarys = $db->escapeString($e_salary); // Escape any input before insert
...
$cx=$db->insert($tab,array('f_name_application'=>$f_namee,'m_name_application'=>$m_namee,
        'l_name_application'=>$l_namee,'gender_application'=>$gendere,
    'religion_application'=>$religione,'birthday_application'=>$birthdaye,
    'national_application'=>$nationale,'marrital_application'=>$marritale,
    'blood_application'=>$bloode,'email_application'=>$emaile,
    'mobile_application'=>$mobilee,'fax_application'=>$faxe,
    'landline_application'=>$landlinee,'address_application'=>$addresse,
    'other_address_application'=>$othere,'code_application'=>$codee,
    'pass'=>$codee."123",'id_categories'=>$id_categoriess,
    'id_departments'=>$id_departmentss,'id_position'=>$id_poss,
    'school'=>$a_schools,'about_pos'=>$a_postions,
    'start'=>$a_starts,'per_salary'=>$p_salarys,
    'exp_salary'=>$e_salarys,'status'=>"pending"));  // Table name, column names and respective values

$res = $db->getResult();  
$id_application=$res[0];

if ($cx == 1) {

    ?> 

        <meta http-equiv="refresh" content="0;url=<?php echo $dir?>?sec=hr&mod=application&action=application_add"/>
              <?php 
echo $done; 
        }
 else{
...
if (isset($_GET['drop']) && $_GET['drop'] == 2)
    {
     $id_cat=$_GET["id_application"];
...
$db->update($tab,array('action' => "desactive",'status' => "reject"),$f);  // Table name, WHERE conditions
$res = $db->getResult(); 
...
$db = new Database();
$db->connect();
$ciox=$db->update( $tab,array('date'=>$date,'status'=>"Appointment"),$conditions); // Table name, column names and values, WHERE conditions
$res = $db->getResult();
...
/*****************************************/
$cx=$db->update( $tab,array('f_name_application'=>$f_name,'m_name_application'=>$m_name,'l_name_application'=>$l_name,
...
$db = new Database();
$db->connect();
$db->update( $tab,array('date'=>$date,'status'=>"Appointment"),$conditions); // Table name, column names and values, WHERE conditions
$res = $db->getResult();
...
$db = new Database();
$db->connect();
$db->select("application",'*',null,"`status` = 'pending'"); // Table name, Column Names, JOIN, WHERE conditions, ORDER BY conditions
$res = $db->getResult();
       } 
        else
            {
