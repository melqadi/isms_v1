...
class Database{
	...
	private $db_name = "isms";	// Change as required
...
// Function to SELECT from the database
	public function select($table, $rows = '*', $join = null, $where = null, $order = null, $limit = null){
		// Create query from the variables passed to the function
		$q = 'SELECT '.$rows.' FROM '.$table.' WHERE `action` != "desactive"  ';
		if($join != null){
			$q .= ' JOIN '.$join;
		}
        if($where != null){
    ...
    public function select_active($table, $rows = '*', $join = null, $where = null, $order = null, $limit = null){
        // Create query from the variables passed to the function
        $q = 'SELECT '.$rows.' FROM '.$table.' WHERE `archived` = "0"  ';
        if($join != null){
            $q .= ' JOIN '.$join;
        }
        if($where != null){
    ...
    public function insertnew($params){
    	// Check to see if the table exists
    	 	$sql=$params;
         $this->myQuery = $sql; // Pass back the SQL
            // Make the query to insert to the database
    ...
    public function delete($table,$where = null){
    	// Check to see if table exists
    	 if($this->tableExists($table)){
    	 	...
    	 	if($where == null){
                ...
                $delete = 'UPDATE '.$table.' SET `action` = "desactive" WHERE '.$where; // Create query to delete rows
              }
	  ...
	  public function deleteall($table,$where = null){
    	// Check to see if table exists
    	 if($this->tableExists($table)){
    	 	// The table exists check to see if we are deleting rows or table
    	 	if($where == null){
    ...
    // archive function 
    public function archive($table,$where = null){
        // Check to see if table exists
        if($this->tableExists($table)){
            // The table exists check to see if we are deleting rows or table
            if($where == null){
                ...
                $delete = 'UPDATE '.$table.' SET `archived` = "1" WHERE '.$where; // Create query to delete rows
            }
            // Submit query to database
            if($del = $this->myconn->query($delete)){
                array_push($this->result,$this->myconn->affected_rows);
